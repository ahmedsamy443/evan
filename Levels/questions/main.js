...
if(scores>=40)
{
    ...
    '<input type="submit" name="Btn_return" value=" تسليم" id="Btn_return" class="btn btn-primary" style="font-size:XX-Large;"  onclick="hello();window.open(\'/evan/dashboard.php\',\'_self\');">' +
    '</div>' +
    '</div>' +
    '</div>';
}    
...
function hello()
{
    ...
    if (scores >= 40) 
    {
        ...
        $.ajax(
            {
                type:"get",
                url:"query_pre.php?score="+scores/*+"&time="+time+"&wimg="+wimg*/,
                success:function()
                {
                    window.open();
                }
    ...
    else
    {
        $.ajax(
            {
                type:"get",
                url:"query_pre.php?score="+scores/*+"&time="+time*/,
